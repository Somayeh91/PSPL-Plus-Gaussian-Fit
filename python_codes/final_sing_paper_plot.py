...
name = 'cassan_16_76_1743.det.lc.gz'  
...
cell_text = [[round(float(t0_theo),2), round(final_param[3],2)] ,[round(float(u0_theo),2),np.abs(round(final_param[4],2))],[round(float(tE_theo),3),round(final_param[5],3)],
...
left, width = 0.1, 0.8
bottom, height_1,height_2, height_3 = 0.07,0.1, 0.77 ,0.2
...
if not len(t_range_p)==0:
    ...
    main.axis([285,1360,min(df['f']),max(df['f'])+(0.05*(max(df['f'])-min(df['f'])))])
    main.axes.get_xaxis().set_ticks([])
    main.set_ylabel('Magnification',fontname='Times New Roman',size=25)
    main.tick_params(axis='y',labelsize=20)
    #main.axvline(x=(round(float(t0_theo)-del_t,2)),color='black', linestyle= "dashed",ymax=0.5929) #dashed line is for "cassan_16_61_1318.det.lc.gz"
    the_table= main.table(cellText=cell_text,colWidths = [0.15]*3,rowLabels=rows, colLabels=columns,loc='best')
    ...
    for tick in main.get_yticklabels():
    ...
    inset.set_title('Assumed Planetary Event',fontname='Times New Roman',size=23)
    ...
    mark_inset(main, inset, loc1=1, loc2=3, fc="none", ec="0.5")
    ...
    residual.axis([285,1360,min(f_res_final)-(0.05*(max(f_res_final)-min(f_res_final))),max(f_res_final)+(0.05*(max(f_res_final)-min(f_res_final)))])
    residual.set_xlabel('Time (Days)',fontname='Times New Roman',size=25)
    residual.tick_params(axis='both',labelsize=17)
    #residual.axvline(x=(round(float(t0_theo)-del_t,2)),color='black', linestyle= "dashed") #dashed line is for "cassan_16_61_1318.det.lc.gz"
    for tick in residual.get_yticklabels():
        tick.set_fontname("Times New Roman")
    for tick in residual.get_xticklabels():
        tick.set_fontname("Times New Roman")
    fig = plt.gcf()
