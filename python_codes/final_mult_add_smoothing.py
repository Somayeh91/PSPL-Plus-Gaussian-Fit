...
def fun2 (mean, sigma,amp, t0,u0,tE,f_s):
    u = np.sqrt(u0**2+((df['t']-t0)/tE)**2)
    A = (((amp/np.sqrt(2*np.pi*(sigma**2)))*np.exp(-((df['t']-mean)**2)/(2*(sigma**2)))))+((u**2)+2)/(u*np.sqrt((u**2)+4))
    return (f_s * (A-1)) +1
...
for i in tqdm( range(500)): #len(dff['name'])
    ...
    if dff['name'][i].endswith('.gz'):
        ...
        if not len(t_range_p)==0:
            ...
            fig = plt.gcf()
            fig.savefig(home+'/Desktop/trial_runs/new_tests/'+str(dff['name'][i])+'.png')
...
result = pd.DataFrame({'name': name_lc ,'q_true_log': q_[0],'q_fitted_log': q_[1],'s_true_log': s_[0],'s_fitted_log': s_[1],
                    ...
                    'u0_true' : u0_[0], 'u0_fitted' : u0_[1], 'f_s_true' : f_s_[0], 'f_s_fitted': f_s_[1], 'chi_2_2' : chi_2_2, 'chi_2_1' : chi_2_1, 'length': length})
                                      
result.to_csv(home+'/Desktop/trial_runs/new_tests/alllc1_100.CSV')
...
fig.savefig(home+'/Desktop/trial_runs/new_tests/alllc1/Mass Ratio_100.png')
plt.close()
plt.figure(4)
...
fig = plt.gcf()
fig.set_size_inches(12.0,6.0)
fig.savefig(home+'/Desktop/trial_runs/new_tests/alllc1/Projected Separation_100.png')
