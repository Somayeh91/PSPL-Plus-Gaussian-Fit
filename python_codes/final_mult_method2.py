...
for i in range(len(tempdata)):
    ...
    if tempdata1.endswith('.gz'):
        ...
        dur_df = pd.DataFrame({'duration' : duration, 'percentage' : jj})
        dur_min = min(dur_df['duration'])
        dur_min_percent = dur_df['percentage'][(dur_df['duration']).argmin()]
        dur_df = pd.DataFrame({'duration' : duration+[dur_min/10,dur_min*10,dur_min*100], 'percentage' : jj+[dur_min_percent/10,dur_min_percent*10,dur_min_percent*100]})
        dur_df['percentage']= dur_df['percentage']*100
        ...
        for g in range(len(dur_df['duration'])):
            sigma = dur_df['duration'][g]
            ...
            for a in range(0,2):
                ...
                min_model_[2].append(dur_df['percentage'][g])
        ...
        percentage_hist.append(min_model_[2][mmm_.argmax()])
        ...
        del_t = round((np.sqrt((float(s)-(1/float(s)))**2 - (float(u0_theo))**2)),3)
        ...
        main.set_title('Simulated WFIRST lc plus the functional modeling for '+'"'+str(tempdata)+'"')
        ...
        fig = plt.gcf()
        fig.savefig(home+'/Library/Mobile Documents/com~apple~CloudDocs/Microlensing/OSU trip/Matt/plots_pandas_residuals2/'+str(tempdata1)+'.png')
        
        plt.close()
        plt.figure(2)
        fig = plt.gcf()
        fig.set_size_inches(12.0,6.0)
        plt.title('Plot of normalized (cumulative chi-squared - N) VS time for '+'"'+str(tempdata)+'"')
        plt.plot(t_chi['t'],t_chi['chi2'],'k.')
        fig.savefig(home+'/Library/Mobile Documents/com~apple~CloudDocs/Microlensing/OSU trip/Matt/plots_pandas_residuals2/'+str(tempdata1)+'_chi.png')


result = pd.DataFrame({'q_true': q_[0],'q_fitted': q_[1],'s_true': s_[0],'s_fitted': s_[1], 'percentage' : percentage_hist})
result.to_csv(home+'/Library/Mobile Documents/com~apple~CloudDocs/Microlensing/OSU trip/Matt/result_file/440_V2.CSV')
plt.close()
plt.figure(3)
plt.title(str(tempdata)+'Mass Ratio_Revision1')
plt.plot(result['q_true'],result['q_true'],'g-',result['q_true'],result['q_fitted'],'b.')
plt.xlabel('True Mass Ratio - log',size=15)
plt.ylabel('Fitted Mass Ratio - log',size=15)
fig = plt.gcf()
fig.set_size_inches(12.0,6.0)
fig.savefig(home+'/Library/Mobile Documents/com~apple~CloudDocs/Microlensing/OSU trip/Matt/plots_pandas_residuals2/Mass Ratio.png')
plt.close()
plt.figure(4)
plt.title(str(tempdata)+'Projected Separation_Revision1')
plt.plot(result['s_true'],result['s_true'],'g-',result['s_true'],result['s_fitted'],'b.')
plt.xlabel('True Separation - log',size=15)
plt.ylabel('Fitted Separation - log',size=15)
fig = plt.gcf()
fig.set_size_inches(12.0,6.0)
fig.savefig(home+'/Library/Mobile Documents/com~apple~CloudDocs/Microlensing/OSU trip/Matt/plots_pandas_residuals2/Projected Separation.png')
end = time.time()
